...
The `kkowalskireef/compute-horde-validator-runner` docker image contains a `docker-compose.yml` file with all the necessary services to run a validator. 
A `watchtower` container that will automatically apply updates for containers.

```
kkowalskireef/compute-horde-validator-runner
|__postgres
|__redis
|__worker
|__...
...
2) `kkowalskireef/compute-horde-validator-runner` container itself, which will automatically update ALL the other containers.
